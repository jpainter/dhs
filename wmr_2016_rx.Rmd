...
saveRDS( data, "x.AM_trend.3b.rds")
...
```{r, fig.width= 7}
 ...
 data = readRDS("x.AM_trend.3b.rds") 
...
data = read_csv("percent ACT by year.csv")

ylabel = "Proportion of febrile children given an antimalarial drug"

fig.3.16.trend = 
    ggplot( data , 
        aes( endyear, percent, group = provider, color = provider, label = country)
        ) + 
  # guides(color = guide_legend()) +
  scale_y_continuous( ylabel , label = percent)  +
  scale_x_discrete("" ) +
...
fig.3.16.trend
  ...
  library(plotly) # use development version of plotly and scales to avoid conflict with newer version of ggplot
  ggplotly(fig.3.16.trend, tooltip = c("label", "x", "y"))
