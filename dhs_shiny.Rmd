...
svy = load_survey_file( .country = country,
                    .survey = survey ,
                    .year = year, 
                    geo = FALSE,
                    design = TRUE ,
...
svy =  update( svy, 
               one = 1,
               hv105.grp.c = cut( hv105, breaks= c(0,5,15,25,35,45, Inf))
               )
return( svy )
})
...
svby_hml32_by_age = reactive({
  ...
  if ( is.null( svy()) ){ return()}
    ...
    svy_obj = svyby( ~ hml32 , ~ hv105.grp.c, svy(), svymean, na.rm = TRUE )
    return(svy_obj)
}) 
...
svby_hml12.anynet = reactive({
  # svytable(~ hml12.anynet + v025, svy()[[1]], round=TRUE) 
  svymean( ~ hml12.anynet , svy() , na.rm = TRUE  )
  # svyby( ~ hml12.anynet ,  ~ one, svy()[[1]] , svymean ,  na.rm = TRUE  )
})
...
svby_hml12.anynet.urban = reactive({
  # svytable(~ hml12.anynet + v025, svy()[[1]], round=TRUE) 
  svyby( ~ hml12.anynet ,  ~hv025, svy() , svymean ,  na.rm = TRUE  )
})
...
svby_hml12.anynet_by_age = reactive({
  # svytable(~ hml12.anynet + v025, svy()[[1]], round=TRUE) 
  svyby( ~ hml12.anynet ,  ~hv105.grp, svy() , svymean ,  na.rm = TRUE  )
})
...
svby_hml12.anynet_by_region = reactive({
  # svytable(~ hml12.anynet + v025, svy()[[1]], round=TRUE) 
  svyboxplot( hml12.anynet~hv024, svy() , ylab = "Slept Under a Net" , xlab= 'Region' , na.rm = TRUE)
})
...
svby_hml12.itn_by_age = reactive({
  # svytable(~ hml12.anynet + v025, svy()[[1]], round=TRUE) 
  svyby( ~ hml12.itn ,  ~hv105.grp, svy() , svymean ,  na.rm = TRUE  )
})
