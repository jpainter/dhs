...
- Survey data, subset to those with microscopy OR RDT results.  When parastemia done by RDT and microscopy, the mean of the two is shown.   

```{r,  include = FALSE}

source('_Code/file_list.r')
...
wmr_pop <- read_excel("../WMR/SEE_WMR2016_MalariaData Oct 6.xlsm", 
  ...
  select( Country, ISO_Ctry, Year, Under5, `Population risk rate (Total)`) %>%
  filter( ISO_Ctry %in% subsahara$iso2c , Year >= 2006  ) %>%
  rename( pop = Under5, endyear = Year, country = Country, iso2c = ISO_Ctry) %>%
  mutate(
...
- the size of the dot is proportional to the year of the survey; **the larger the dot the more recent the survey.**
...
d_all = left_join( wmr_incidence , all_parasitemia %>% select(-country), by = c("iso2c", "endyear") ) %>%
  filter( !is.na(id) ) %>%
  mutate( label = paste( country, endyear ) )

# View(all_parasitemia)
# View( d_all )
...
gg = ggplot(d_all, aes( prevalence, inc_total, color = country , label = label)) +
  ...
  labs( x = "prevalence (most recent survey)", y = "Presumed and confirmed cases (1/PYO)" ,
        title = "Incidence v. Prevalence \n(hover to see details)") +
  guides( color = FALSE, size = FALSE, group = FALSE , label = FALSE)

ggp = ggplotly( gg , tooltip = "label")
