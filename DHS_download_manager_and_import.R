...
# Get list of files from download manager text ####
...
manager_list = read_csv("download_list_Oct2016")
# str(manager_list)
nrow(manager_list)
...
skip.these.iso2c = countrycode(skip.these, "country.name", "iso2c")
skip_string = paste0("Code=", skip.these.iso2c, collapse = "|")
file_list = manager_list %>% filter( !grepl( skip_string , URL ))
nrow(file_list)
...
## loop through each available url #####
  
for ( j in seq_along( file_list) ){
  
  url <- file_list[ j, "URL" ] %>% as.character()
  
  country_code_start = regexpr("Code=", url)[1]
  country_iso2c = substr(url, country_code_start + 5, country_code_start + 6)
  country_name = countrycode( country_iso2c, "iso2c", "country.name")
  
  cat( country_name, "\n") # JP
  
  # create the country directory on the local disk
  parent_dir = "dhs_download"
  dir.create( paste0( parent_dir , "/",  country_name ), showWarnings = FALSE )
  
  setInternet2(TRUE) 
  url_pass = "http://jpainter@cdc.gov:maynard@dhsprogram.com/customcf/legacy/data/download_dataset.cfm?Filename=AOBR61DT.zip&Tp=1&Ctry_Code=AO&surv_id=395&dm=1"
  zz = url(url)
  readLines(zz)
  
  require(R.utils)
  downloadFile(url, "myfile.test", username = "jpainter@cdc.gov", password ="maynard")
  
  download.file( url, destfile = paste0( parent_dir , "/",  country_name , "/test.file") , method = 'auto')
  
     # access each dataset's link
    z <- GET( url) 
    
    # read the table from each country page, remove the country name, and remove extraneous characters
    this.title <- gsub( ": |," , "" , gsub( country_name , "" , readHTMLTable( content(z, as = "text") )[[1]][1,1] ) )
    
    # create a dataset-specific folder within the country folder within the current working directory
    dir.create( paste0( parent_dir , "/",  country_name , "/" , this.title ) )
