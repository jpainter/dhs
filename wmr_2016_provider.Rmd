...
kable( X %>% count( country, survey, year ) %>% arange(-year) , row.names = TRUE)
```
...
x %>% arrange( -substr(year, 1, 4) ) %>% count( country, survey, year ) %>%  kable( row.names = TRUE)
```
